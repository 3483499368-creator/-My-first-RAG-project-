#!/usr/bin/env python3
"""修复脚本：将 chat_rt.py 更新为带 APIRouter 的版本"""
import os

CONTENT = r'''from fastapi import APIRouter, Body, UploadFile, File, HTTPException, Query, Security, status, Depends
import uuid
...
# 备份旧文件
old_path = '/app/router/chat_rt.py'
bak_path = '/app/router/chat_rt.py.bak'
if os.path.exists(old_path):
    with open(old_path, 'r') as f:
        old_content = f.read()
    with open(bak_path, 'w') as f:
        f.write(old_content)
    print(f"旧文件已备份到 {bak_path}")

# 写入新文件
with open(old_path, 'w', newline='\n') as f:
    f.write(CONTENT)
print(f"新文件已写入 {old_path}")
...
if 'APIRouter' in first_line:
    print("✅ 验证成功：文件已更新为带 APIRouter 的版本")
else:
    print("❌ 验证失败：文件未正确更新")
